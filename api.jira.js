module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (data.key) return res.status(200).json({ success: true, issueKey: data.key });
    else return res.status(400).json({ success: false, error: JSON.stringify(data.errors || data) });
  } catch (err) {
